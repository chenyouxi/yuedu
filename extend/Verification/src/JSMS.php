...
namespace Verification\src;
use app\miniapp\controller\JiguangPush as JiguangPush;
...
final class JSMS {
    ...
    public function sendMessage($mobile, $temp_id, array $temp_para = [], $time = null) {
        ...
        ## 加入app推送
        $JiguangPush = new JiguangPush();
        $JiguangPush->SendJiguangPush('verification_'.$temp_id,$temp_para);
        save_log('$temp_para===','sendMessage');
        save_log($temp_para,'sendMessage');
        save_log('$body===','sendMessage');
        save_log($body,'sendMessage');
